...
public class Authorization : Control
{
    ...
    const string BINDING = "127.0.0.1";
    ...
    private async void authorize()
    {
        ...
        {
            GD.Print("Authorized");
            EmitSignal("playerAuthenticated");
    ...
    private async Task<bool> getTokenFromAuthCode(string authCode)
    {
        ...
        Error Err = httpRequest.Request(tokenServer, header, false, HTTPClient.Method.Post, String.Join("&", bodyPart));
    ...
    private async Task<bool> isTokenValid()
    {
        ...
        HTTPRequest httpRequest = new HTTPRequest();
        AddChild(httpRequest);
        var Err = httpRequest.Request(tokenServer + "/introspect", header, false, HTTPClient.Method.Post, String.Join("&", bodyPart));
